...
class Window(QMainWindow):
	def __init__(self):
		...
		parser = argparse.ArgumentParser(description=("Visualization of infinite dimensional Ising model spin glass") )
		parser.add_argument('-N', type=int,default=4, help='Number of spins')
		parser.add_argument('-loadBC', action="store_true", help='Load bond configuration')
		parser.add_argument('-BC', type=str,default='101011', help='Bond Configuration')
...
def BCtoJij(N, BC):
	clause_list = np.array([np.array(list(c)) for c in it.combinations([i for i in range(N)],2)])
